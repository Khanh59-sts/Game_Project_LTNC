...
// Tạo Character với trạng thái đồ họa và âm thanh
...
// xử lí logic di chuyển, va chạm walls
void Character::CharacterMove(glm::vec2 input_velocity, const std::vector<std::shared_ptr<GameObject>> &walls) {
    auto set_drawable_function = [this](const std::shared_ptr<Core::Drawable> &drawable) {
        this->SetDrawable(drawable); // với vận tốc và các bức tường để "Collider"
    };


    auto isGrounded = GroundCheck(walls);
    if (!isGrounded) { // Đang nhảy
        rigidbody_.AddAcceleration({0, gravity_});
        if (is_direction_right_) { // kiểm tra hướng di chuyển
    ...
    } else { // Đang trên mặt đất -> chạy/Idle 
    ...
    if (input_velocity.x > 0) { // cập nhật hướng di chuyển
    ...
    if (is_run_ && isGrounded) {
        audio_manager_.Play(AudioManager::SFX::Run); // phát âm thanh của "chạy"
    }
    if (input_velocity.y > 0 && isGrounded) {
        rigidbody_.AddAcceleration({0, jump_height_});
        audio_manager_.Play(AudioManager::SFX::Jump); // phát âm thanh của "nhảy"
    }
    rigidbody_.SetVelocity(
            {move_speed_ * input_velocity.x * Util::Time::GetDeltaTimeMs() / 1000, rigidbody_.GetVelocity().y}); // cập nhật lại tốc độ để đảm bảo cân bằng tốc độ
}
...
// cập nhật trạng thái và logic "TRONG MỖI KHUNG HÌNH"
void Character::Update(glm::vec2 input_velocity, const std::vector<std::shared_ptr<GameObject>> &walls) {
    ...
    rigidbody_.Update(GetCollider(), walls, translate); // cập nhật vị trí dựa trên va chạm
}
